        ...
        Runnable titlescreen = new Runnable() {
			...
			public void run() {
		        ...
		        Core.getRootManager().add(new InstantView(ae = new AbstractEntity() {
					...
					@Override
					public void update() {
						thus.get(PositionComponent.class).position = thus.get(PositionComponent.class).position.add
								(new Vector2(Core.getDefaultTimer().getDeltaTime()*50.0d, Core.getDefaultTimer().getDeltaTime()*10.0d));
