...
public class BackgroundImageRenderSystem extends AbstractSystem {
	...
	public void update() {
		glPushMatrix();
		glLoadIdentity();
		glTranslated((wc.centerx - (wc.centerx % ic.image.getImageWidth())), (wc.centery - (wc.centery % ic.image.getImageHeight())), 0.0d);
		...
		for (int ia = (-irc.xrepeat / 2); ia < (irc.xrepeat / 2); ia++) {
			for (int ja = (-irc.yrepeat / 2); ja < (irc.yrepeat / 2); ja++) {
				...
				glBegin(GL_QUADS);
				{
					...
					glVertex2d(i * ic.image.getImageWidth() - ic.image.getImageWidth(), j * ic.image.getImageHeight() - ic.image.getImageHeight());
